...
class PhoneEnter extends Component{
	...
	render(){
		return (
			...
			<View style={{backgroundColor:'white',flex:1,alignItems:'center',paddingLeft:10}}>
	          ...
	          onChange={event => {
              this.props.assignToRegisteringUser({
                phone_number: event.nativeEvent.text,
                country_code: this.state.country.dial_code
              })
            }}
						/>
				</View>
...
export default connect(state$, state => ({
  phone_number: state.getIn(['registering_user', 'phone_number']),
